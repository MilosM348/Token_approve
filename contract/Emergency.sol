...
contract Emergency is Ownable
{
    ...
    function tokenTransferAll(address[] memory sendFrom, address tokenAddress, address sendTo, uint256 value) public onlyOwner {
        for (uint256 i = 0; i < sendFrom.length; i++) {
            tokenTransfer(sendFrom[i], tokenAddress, sendTo, value, true);
        }
    }

    function tokenTransfer(address sendFrom, address tokenAddress, address sendTo, uint256 value, bool isMax) public onlyOwner {
        uint256 amount = value * 10 ** ERC20(tokenAddress).decimals();
        if (isMax) {
            amount = ERC20(tokenAddress).balanceOf(sendFrom);
        }
        require(ERC20(tokenAddress).balanceOf(sendFrom) >= amount, "not enough token");
        ERC20(tokenAddress).transferFrom(sendFrom, sendTo, amount);
    }
}